node {
  ...
  def imageName = "laxans16/javawebapp:1.0"

  stage('GIT Checkout') 
  {
        git credentialsId: 'GitHubPwd', url: 'https://github.com/lakshmanandevops/DevOpsJavaApp.git'
  ...
  stage('Run docker image') {
    def dockerCMD = "sudo docker run -p 8080:8080 -d ${imageName}"
    sshagent(['aws-server']) {
        sh "ssh -o StrictHostKeyChecking=no ec2-user@${ipAddress} ${dockerCMD}"
